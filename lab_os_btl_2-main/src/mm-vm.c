...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr)
{
  ...
  int inc_sz = PAGING_PAGE_ALIGNSZ(old_sbrk + size) - PAGING_PAGE_ALIGNSZ(old_sbrk);
...
int __free(struct pcb_t *caller, int vmaid, int rgid)
{
  ...
  // Find rgnode in freerg_list
	...
	while (rgit != NULL)
	{
    if (cur_vma->vm_freerg_list->rg_start == cur_vma->vm_freerg_list->rg_end && cur_vma->vm_freerg_list->rg_next == NULL)
    {
      //empty free list
      cur_vma->vm_freerg_list->rg_start = rgnode.rg_start;
      cur_vma->vm_freerg_list->rg_end = rgnode.rg_end;
      break;
    }
		struct vm_rg_struct *nextrg = rgit->rg_next;
		if (nextrg == NULL)
		{
      if (rgit->rg_start == rgnode.rg_end)
      {
        //node->free(rgit)->(used)->NULL
        rgit->rg_start = rgnode.rg_start;
      }
      else if (rgit->rg_start > rgnode.rg_end)
      {
        //node->used->free(rgit)->(used)->NULL
        rgnode.rg_next = rgit;
        rgit = &rgnode;
      }
			else if (rgit->rg_end == rgnode.rg_start)
			{
        //free(rgit)->node->NULL
		...
		else
		{
			...
			{
				//free(rgit)->used->node->free(nextrg)
...
int __read(struct pcb_t *caller, int vmaid, int rgid, int offset, BYTE *data)
{
  ...
  if (currg->rg_end < offset)
  {
    printf("Invalid access... READ instruction is not accepted.\n");
    return -1;
  }
...
int __write(struct pcb_t *caller, int vmaid, int rgid, int offset, BYTE value)
{
  ...
  if (currg->rg_end < offset)
  {
    printf("Invalid access... WRITE instruction is not accepted.\n");
    return -1;
  }
