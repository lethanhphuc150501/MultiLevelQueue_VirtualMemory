...
{                                         // no guarantee all given pages are mapped
   ...
   */
  ret_rg->rg_end = addr + pgnum*PAGING_PAGESZ;  //Extended region
  ...
  while (fpit != NULL)
  {
    ...
    caller->mm->pgd[pgn+pgit] |= (1 << 31); //present bit
    fpit = fpit->fp_next;
...
int alloc_pages_range(struct pcb_t *caller, int addr, int req_pgnum, struct framephy_struct** frm_lst)
{
  int pgit, fpn;
  int pgn = PAGING_PGN(addr);
  ...
  for(pgit = 0; pgit < req_pgnum; pgit++)
  {
    ...
    {  // ERROR CODE of obtaining somes but not enough frames      
      ...
      pte_set_swap(&caller->mm->pgd[pgn+pgit], 0, swpfpn);
    }
  }
  return 0;
}
...
int vm_map_ram(struct pcb_t *caller, int astart, int aend, int mapstart, int incpgnum, struct vm_rg_struct *ret_rg)
{
  ...
  ret_alloc = alloc_pages_range(caller, mapstart, incpgnum, &frm_lst);
